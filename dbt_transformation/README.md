...
To produce the data products requested by the clients, several metrics should be created from the dimensional models. Before the introduction of semantic layer by DBT, the metrics could be created by directly querying from the models or by calculating the metrics outside of the DBT environment e.g. in the dashboard (for DA) or jupyter notebook (for DS). While it is arguably the most convenient way to do such a thing, the second option is quite dangerous practice that we want to avoid since this can lead to confusion and multiple interpretation to the metrics, especially when the team members grow and the pipeline becomes more and more complex.

As a solution to tackle this organizational complexity problem, DBT now owns semantic layer capabilities for standardizing metrics definition. In order to do that, `semantic_model.yml` and `metrics.yml` files are created under the `semantic_models` directory. These files will give semantic meaning to the models by stating the **measures**, **entities**, and **dimensions** prior to the metrics calculation. By doing this, each time the users want to query a metric, for example `Marketing to Budget Revenue Ratio (MBTR)` they can run:
```bash
